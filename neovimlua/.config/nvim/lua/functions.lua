...
function _G.rename(bufnr)
    ...
    for _, n in ipairs(locals.find_usages(definition, scope, bufnr)) do nodes_to_rename[n:id()] = n end
...
function _G.Query()
    ...
    local matches = query.get_capture_matches(bufnr, "@definition.function", "locals", nil, "elixir")
    for _, n in ipairs(matches) do P(ts_utils.get_node_text(n.node)) end
...
function _G.ExTest()
    ...
    if not string.find(file, ".ex") then return end
